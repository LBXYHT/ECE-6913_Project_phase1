    ...
    InsMem(string name, string ioDir) {
        ...
        if (imem.is_open()) {
            ...
            while (getline(imem, line)) {
                if (line != "11111111") line.pop_back();
    ...
    void InstructionDecode() {
        ...
        if (!state.ID.nop) {
            ...
            if (OPCODE == "0110011") {
                ...
                state.EX.Rs = bitset<5>(line.substr(12, 5));
                state.EX.Read_data1 = myRF.readRF(state.EX.Rs);
                state.EX.Imm = bitset<16>(signextend(line.substr(0, 12), 16));
                state.EX.rd_mem = false;
                state.EX.wrt_mem = false;
                ...
                state.EX.Imm = bitset<16>(signextend(line.substr(0, 1) + line.substr(12, 8) + line.substr(11, 1) + line.substr(1, 10) + '0', 16));
                state.EX.rd_mem = false;
                state.EX.wrt_mem = false;
                state.EX.is_I_type = false;
                ...
                state.EX.Rt = bitset<5>(line.substr(7, 5));
                state.EX.Read_data2 = myRF.readRF(state.EX.Rt);
                state.EX.Imm = bitset<16>(signextend(line.substr(0, 1) + line.substr(24, 1) + line.substr(1, 6) + line.substr(20, 4) + '0', 16));
                state.EX.rd_mem = false;
                state.EX.wrt_mem = false;
                state.EX.is_I_type = false;
                ...
                state.EX.Rt = bitset<5>(line.substr(7, 5));
                state.EX.Read_data2 = myRF.readRF(state.EX.Rt);
                state.EX.Imm = bitset<16>(signextend(line.substr(0, 7) + line.substr(20, 5), 16));
                state.EX.rd_mem = false;
...
public:
    ...
    void InstructionExecute() {
        ...
        if (!state.EX.nop) {
            ...
            if (state.EX.alucontrol.to_string() == "0010") {
                //add operation
                nextstate.MEM.ALUresult = state.EX.Read_data1.to_ulong() + Aluin2;
            } else if (state.EX.alucontrol.to_string() == "0110") {
                //sub operation
                nextstate.MEM.ALUresult = state.EX.Read_data1.to_ulong() - Aluin2;
            } else if (state.EX.alucontrol.to_string() == "0000") {
    ...
    void InstructionDecode() {
        ...
        if (!state.ID.nop) {
            ...
            if (OPCODE == "0110011") {
                ...
                nextstate.EX.Rs = bitset<5>(line.substr(12, 5));
                nextstate.EX.Read_data1 = myRF.readRF(nextstate.EX.Rs);
                nextstate.EX.Imm = bitset<16>(signextend(line.substr(0, 12), 16));
                nextstate.EX.rd_mem = false;
                nextstate.EX.wrt_mem = false;
                ...
                nextstate.EX.Read_data2 = 0;
                //imme [20|10:1|11|19:12]
                nextstate.EX.Imm = bitset<16>(signextend(line.substr(0, 1) + line.substr(12, 8) + line.substr(11, 1) + line.substr(1, 10) + '0', 16));
                nextstate.EX.rd_mem = false;
                nextstate.EX.wrt_mem = false;
                nextstate.EX.is_I_type = false;
                ...
                nextstate.EX.Rt = bitset<5>(line.substr(7, 5));
                nextstate.EX.Read_data2 = myRF.readRF(nextstate.EX.Rt);
                nextstate.EX.Imm = bitset<16>(signextend(line.substr(0, 1) + line.substr(24, 1) + line.substr(1, 6) + line.substr(20, 4) + '0', 16));
                nextstate.EX.rd_mem = false;
                nextstate.EX.wrt_mem = false;
                nextstate.EX.is_I_type = false;
                ...
                nextstate.EX.Rt = bitset<5>(line.substr(7, 5));
                nextstate.EX.Read_data2 = myRF.readRF(nextstate.EX.Rt);
                nextstate.EX.Imm = bitset<16>(signextend(line.substr(0, 7) + line.substr(20, 5), 16));
                nextstate.EX.rd_mem = false;
            ...
            if (OPCODE == "1100011") {
                if (nextstate.EX.funct3 == "000") {
                    ...
                    if ((long)nextstate.EX.Read_data1.to_ulong()-(long)nextstate.EX.Read_data2.to_ulong() !=0) {
                        nextstate.IF.PC = state.IF.PC.to_ulong() + static_cast<long>(static_cast<int16_t>(nextstate.EX.Imm.to_ulong())) - 4;
                        nextstate.IF.nop = false;
                        state.IF.nop = true;
                    } 
